...
- **Three named steps, in order: schema pass, freeze, results.** A reading rule cannot be written without looking at the data's formats, so the look is a declared step with an artefact, not a private glance. The *schema pass* inspects columns, spellings, blank rates and row counts and produces a committed archive schema report (`scripts/schema-report`, `archives/<name>/`); it computes no figure. The *freeze* is `scripts/freeze`, which witnesses the declaration against roots we do not control and commits it with its proofs; the declaration states the schema pass's scope and cites the report's digest. *Results* come only after the freeze, from a runner that refuses to change a committed row. A declaration written against an archive with no schema report is not frozen; it is guessed.
- **The record for forensic investigations from 012 onward is git plus external witnesses**: the frozen declaration with its OpenTimestamps and RFC 3161 proofs, the committed evidence with source digests, and `scripts/check` (rule-to-test map, proof verification). That is what a stranger can verify without trusting this machine. Morpholog holds the governed research record (metrics with provenance, the forward-mystery lineage, and from 25/09/2026 every dated watch, which `check-record` refuses to let lapse). **A standing instrument is a record engine once its runner's output can be shown to change silently** (the kill test of 25/09/2026, `bill/KILL-TEST.md`): the runner keeps computing under its frozen declaration and amendments, and a separate Morpholog record, with its own database, signing key and DigiCert-witnessed checkpoints, holds each computed row once per rule version and refuses to let it change (`tec/`, `bill/`). Nothing is backfilled: a row is recorded when it is computed, under the rule version it was computed under, and the record says so.
...
- **Morpholog owns the governed research record for metrics with provenance, dated watches and the forward-mystery lineage**; standing instruments and forensic investigations from 012 onward are recorded in git with external witnesses (see the doctrine). Do not turn Morpholog into the analytics engine.
...
- **Archive schema reports live under `archives/<name>/`** (`SCHEMA.md` and `schema-report.json`), regenerated by `scripts/schema-report` and committed; a declaration cites the digest it was written against.
- **The frozen declaration lives in its own file** (`DECLARATION.md`, or `DECLARATION-v<n>.md` for a restated series), never the document that later grows results. Its SHA-256 is what the governed record seals, so those bytes must remain recoverable forever; `scripts/check-record` enforces that every sealed digest still matches a file on disk. Keep the canonical method in tested Python modules, not in an opaque notebook.
