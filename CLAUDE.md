...
- **The record for standing instruments and for forensic investigations from 012 onward is git plus external witnesses**: the frozen declaration with its OpenTimestamps and RFC 3161 proofs, the committed evidence with source digests, and `scripts/check` (rule-to-test map, proof verification). That is what a stranger can verify without trusting this machine. Morpholog remains the record for governed metrics with provenance, dated watches and the forward-mystery lineage, and is not extended to standing instruments after the fact; the two records are revisited when Morpholog's externally witnessed checkpoints (issue #321) exist, because then the runtime can offer the same root of trust plus runtime-defended arithmetic.
