...
as a pure function of `evidence/series.json`. Its declaration is witnessed by
OpenTimestamps and two RFC 3161 authorities (`scripts/timestamp`, verified
offline by `scripts/verify-timestamp` against the roots in `trust/tsa/`). The
series is computed from the archive of TEC Register copies under
`data/raw/neso/tec-history/` (local, journalled, digest-pinned), which NESO's
own portal cannot reproduce because it publishes only the latest copy.
